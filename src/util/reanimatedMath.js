...
const { eq, set, cond, atan, add, multiply, lessThan, abs, divide, sub, min: min2, max: max2, round: reRound, greaterThan, pow, and, greaterOrEq, lessOrEq, proc, floor, } = Animated;
export const bin = (value) => (value ? 1 : 0);
export const fract = (x) => sub(x, floor(x));
export const sign = (x) => cond(lessThan(x, 0), -1, cond(eq(x, 0), 0, 1));
export const min = (...args) => args.reduce((acc, arg) => min2(acc, arg));
export const max = (...args) => args.reduce((acc, arg) => max2(acc, arg));
export const minus = (x) => multiply(-1, x);
export const avg = (...v) => divide(add(...v), v.length);
export const clamp = proc((value, lowerBound, upperBound) => min2(max2(lowerBound, value), upperBound));
export const between = (value, lowerBound, upperBound, inclusive = true) => {
    if (inclusive) {
...
export const approximates = proc((a, b, precision = 0.001) => lessThan(abs(sub(a, b)), precision));
export const toRad = proc((deg) => multiply(deg, Math.PI / 180));
export const toDeg = proc((rad) => multiply(rad, 180 / Math.PI));
...
export const cubicBezier = proc((t, p0, p1, p2, p3) => {
    const term = sub(1, t);
...
export const round = proc((value, precision = 0) => {
    const p = pow(10, precision);
    return divide(reRound(multiply(value, p)), p);
});
export const inc = proc((value) => set(value, add(value, 1)));
export const dec = proc((value) => set(value, sub(value, 1)));
