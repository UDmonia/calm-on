...
const Grid = ({ filter, shopView, currentItems, selectOrDeselect, cart }) => {
  ...
  const renderItems = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        onPress={() => selectOrDeselect(item)}
        style={[styles.gridItem, cart[item.sub].id === item.id && activeStyle]}
      >
        <View style={styles.gridItemTop}>
          <Image style={styles.icon} source={item.icon} />
        </View>
        <View style={styles.gridItemBottom}>
          {shopView ? (
            <>
              <Image
                style={styles.coin}
                source={require("../../assets/cashShop/coin.png")}
              />
              <Text style={[styles.text, styles.price]}>{item.cost}</Text>
            </>
          ) : (
            <Text>{cart[item.sub].id === item.id ? "EQUIPPED" : "EQUIP"}</Text>
          )}
        </View>
      </TouchableOpacity>
    );
  ...
  const renderItem = ({ item, index }) => {
    return (
      <TouchableOpacity
        ...
        style={[styles.category, item === current && activeStyle]}
      >
...
const AccessoryView = () => {
  ...
  const [currentFilter, changeFilter] = useState(
  ...
  */
  ...
  useEffect(() => {
    ...
    const newItems = storeData.auora.currentStorePieces.filter((item) => {
      ...
      if (shopView) {
        item["purchased"] =
          purchasedItemsHash != null &&
          purchasedItemsHash.has(item.id) === true;
      } else {
        if (
          purchasedItemsHash != null &&
          purchasedItemsHash.has(item.id) === false
        ) {
          return;
        }
      }
  ...
  return (
    <View style={styles.main}>
      <View style={styles.previewContainer}>
        <ImageBackground
          style={{ flex: 1 }}
          source={require("../../assets/cashShop/background.png")}
        >
          <View style={styles.previewInnerContainer}>
            <View style={styles.previewLeft}>
              <Image
                resizeMode="contain"
                style={styles.avatar}
                source={require("../../assets/cashShop/auora/auora_base.png")}
              />
            ...
            <View style={styles.previewRight}>
              ...
              {totalCost > 0 && shopView && (
                <TouchableOpacity
                  ...
                  onPress={() => isCheckout(true)}
                >
                  <Text style={styles.previewText}>Buy</Text>
                </TouchableOpacity>
              )}
            </View>
          </View>
      ...
      {checkout && (
        <CheckoutModal
          emptyCart={setSelected}
          cost={totalCost}
          itemList={convertCartToList(selected)}
          byOutfit={false}
          checkout={checkout}
      ...
      <View style={styles.categories}>
        <Categories
          setCurrent={setIndex}
          current={categoryList[currentCategoryIndex]}
        />
      </View>
